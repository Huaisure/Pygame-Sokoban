...
class Solution:
    def __init__(self, game_info: dict) -> None:
        ...
        # self.target_positions = self.get_target_positions()  # 目标位置
    ...
    def parse_game_info(self, game_info: dict) -> None:
        """
        解析游戏信息
        :return: None
        """
        self.game_info = game_info
        self.matrix_size = self.game_info["matrix_size"]
        self.layout = self.game_info["layout"]
        self.pos = self.game_info["player_pos"]
        self.box_positions = self.game_info["box_positions"]
        self.target_positions = self.game_info["target_positions"]
        self.box_target = self.game_info["box_target"]
    ...
    def is_valid_move(
        self, cur_pos: tuple, cur_boxes: List[Tuple[int, int]], dir: Direction
    ) -> bool:
        """
        判断是否为合法的移动
        :param cur_pos: 当前位置
    ...
    def get_possible_moves(
        self, pos: tuple, boxes: List[Tuple[int, int]]
    ) -> List[Direction]:
        """
        获取可能的移动
        :param pos: 当前位置
    ...
    def is_pruned(self, box: Tuple[int, int]) -> bool:
        ...
        x, y = box
        # 如果箱子在目标位置上，则不剪枝
        if box in self.target_positions:
            return False
    ...
    def is_solved(self, cur_boxes: List[Tuple[int, int]]) -> bool:
        ...
        :return: 是否解决
        """
        return set(cur_boxes) == set(self.target_positions)
